...
export default function EditFeelingPage() {
  ...
  const [formData, setFormData] = useState<Partial<CreateFeelingInput>>({
    ...
    preview: "",
    reminder: "",
    quran: { text: "", reference: "", suraNumber: null, verseNumber: null },
  ...
  useEffect(() => {
    const fetchFeeling = async () => {
      try {
        const response = await api.getAdminFeelingById(id);
        if (response.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const cleanedData = {
        ...formData,
        actions: formData.actions?.filter((a: string) => a.trim() !== "") || [],
      };
          ...
          <div className="backdrop-blur-xl bg-white/5 rounded-2xl border border-white/10 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelClass}>Sura #</label>
                  <input
                    type="number"
                    min="1"
                    max="114"
